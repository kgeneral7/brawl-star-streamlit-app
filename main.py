...
# ----------------- (A) 收割機專用函式 (背景化改造) -----------------
def log_message(msg):
    """不再需要傳入 UI 容器，直接寫入快取，讓背景執行緒完全獨立"""
...
def render_home():
    ...
    st.markdown("歡迎來到終極戰術大廳！請在下方輸入您的兩把專屬 API 金鑰。**系統會自動將它們分配給對應的子系統。**")
    
    col1, col2 = st.columns(2)
    with col1:
        st.info("⚙️ **排位收割機專用**")
        bs_input = st.text_input("🔑 Brawl Stars API Key (Supercell 官方核發)", type="password", value=st.session_state.bs_api_key)
        if bs_input != st.session_state.bs_api_key:
            st.session_state.bs_api_key = bs_input
            st.success("Brawl Stars 金鑰已綁定！")
    with col2:
        st.info("🧠 **BP AI 分析專用**")
        gemini_input = st.text_input("🔑 Google Gemini API Key (Google AI Studio 核發)", type="password", value=st.session_state.gemini_api_key)
    ...
    with col_a:
        ...
        st.write("在背景全自動巡邏神仙房，收集最高端的對戰數據與勝率。支援**「全域背景運作」**，掛機時可隨意切換至 BP 大廳使用！")
    with col_b:
...
def render_scraper():
    ...
    with col2:
        if st.button("⏹️ 緊急停止並保留資料", disabled=not st.session_state.is_running, use_container_width=True, type="primary"):
            st.session_state.is_running = False # 背景執行緒看到這個變數改變，就會自動停工
            st.rerun()
    with col3:
    ...
    if st.session_state.is_running:
        st.info("🔄 收割機背景全速運作中... (您可以隨時切換左側導航至【BP 指示器】，收割進度不會中斷！)")
        st.code("\n".join(st.session_state.logs[-25:]), language="plaintext")
        
        # 每 1.5 秒自動重整一次此分頁，以抓取背景的最新日誌
        time.sleep(1.5)
        st.rerun()
