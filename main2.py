...
if "scraper_mode" not in st.session_state:
    st.session_state.scraper_mode = "rooms"
...
def harvest_rooms(headers, duration):
    ...
    log_message(
        f"🚀 【模式 A：神仙房間陣容版】啟動！預計執行到: {end_time.strftime('%H:%M:%S')}"
    )
    ...
    while datetime.now() < end_time and player_queue and st.session_state.is_running:
        ...
        if iteration_count % 5 == 0:
            ...
            log_message(
                f"⏱️ 進度: {iteration_count} 玩家已檢查 | 精英玩家: {elite_found} | 累積對戰數: {len(st.session_state.data)} | 已用時間: {elapsed_time:.1f}分鐘"
            )
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                prof_res = requests.get(
                    f"https://api.brawlstars.com/v1/players/{current_tag}",
                    headers=headers,
                    timeout=10,
                )
                if (
                    ...
                    elite_found += 1
                    log_message(
                        f"[{datetime.now().strftime('%H:%M:%S')}] 🏆 找到神仙 {raw_tag}！"
                    )
                    for item in data:
                        battle = item.get("battle", {})
            ...
            pass
        time.sleep(0.3)
        if len(player_queue) > 10000:
            player_queue = set(random.sample(list(player_queue), 5000))
...
def harvest_solo(headers, duration):
    ...
    end_time = datetime.now() + timedelta(minutes=duration)
    log_message(
        f"🚀 【模式 B：絕頂單人英雄勝率】啟動！預計執行到: {end_time.strftime('%H:%M:%S')}"
    )
    ...
    while datetime.now() < end_time and player_queue and st.session_state.is_running:
        ...
        if iteration_count % 5 == 0:
            elapsed_time = (datetime.now() - st.session_state.start_time).seconds / 60
            log_message(
                f"⏱️ 進度: {iteration_count} 玩家已檢查 | 精英玩家: {elite_found} | 已用時間: {elapsed_time:.1f}分鐘"
            )
        current_tag = random.choice(list(player_queue))
        player_queue.remove(current_tag)
        visited_players.add(current_tag)
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                if (
                    ...
                    elite_found += 1
                    log_message(
                        f"[{datetime.now().strftime('%H:%M:%S')}] 🏆 大神 {raw_tag}！"
                    )
                    for item in data:
                        battle = item.get("battle", {})
                        if battle.get("type") not in ["soloRanked", "teamRanked"]:
...
def render_home():
    ...
    with col_a:
        ...
        st.write(
            "在背景全自動巡邏神仙房，收集最高端的對戰數據與勝率。支援**「全域背景運作」**，掛機時可隨意切換至 BP 大廳使用！"
        )
    with col_b:
        st.subheader("🤖 系統二：BP 即時戰術指示器")
        st.write(
            "載入收割機產出的 CSV，在排位賽 BP 階段提供超光速的選角與 Ban 角建議。"
        )
...
def render_scraper():
        ...
        if st.button(
            ...
            if not st.session_state.bs_api_key:
                ...
                st.session_state.export_filename = (
                    f"brawl_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
                )
    ...
    if st.session_state.is_running:
        st.info(
            "🔄 收割機背景全速運作中... (您可以隨時切換左側導航至【BP 指示器】，收割進度不會中斷！)"
        )
    ...
    if not st.session_state.is_running:
        if (
            st.session_state.scraper_mode == "solo"
            and len(st.session_state.solo_stats) > 0
        ):
            export_rows = []
            ...
            if export_rows:
                st.session_state.data = export_rows
            st.session_state.solo_stats.clear()

        if st.session_state.data:
            st.subheader("📊 結算結果")
            st.success(
                f"✅ 成功存檔！共收集/整理了 {len(st.session_state.data)} 筆數據。"
            )
            csv_data = generate_csv(
                st.session_state.data, st.session_state.scraper_mode
            )
            st.download_button(
                label="⬇️ 點此下載 CSV 報表",
...
def render_bp():
    ...
    with st.sidebar:
        ...
        uploaded_files = st.file_uploader(
            "請選擇一個或多個 CSV 檔案", type=["csv"], accept_multiple_files=True
        )
        if uploaded_files:
            if st.button(
                "📥 融合載入已選檔案", type="primary", use_container_width=True
            ):
    ...
    if st.button(
        ...
        if not st.session_state.gemini_api_key:
            ...
            for i, s in enumerate(top_winrate, 1):
                prompt_data += (
                    f"{s['name']} (勝率 {s['win_rate']}%, 出場 {s['plays']} 次)\n"
                )
            ...
            with st.status("🧠 啟動超光速分析與 AI 連線中...", expanded=True) as status:
                try:
                    ...
                    if list_res.status_code != 200:
                        raise Exception(
                            f"無法取得模型清單，請確認 API Key。錯誤碼：{list_res.status_code}"
                        )
                    ...
                    if not target_model:
                        raise Exception("找不到任何支援對話的 Gemini 模型！")
                    ...
                    for attempt in range(max_retries):
                        try:
                            ...
                            if response.status_code == 200:
                                ai_reply = response.json()["candidates"][0]["content"][
                                    "parts"
                                ][0]["text"]
                                break
                                ...
                                if attempt < max_retries - 1:
                                    wait_sec = 15
                                    status.write(
                                        f"⚠️ 伺服器忙碌 (錯誤碼 {response.status_code})，系統於 {wait_sec} 秒後自動重試..."
                                    )
                                    time.sleep(wait_sec)
                                    continue
                                else:
                                    raise Exception(
                                        f"Google 伺服器持續塞車 ({response.status_code})。請稍後再試！"
                                    )
                            else:
                                raise Exception(
                                    f"伺服器錯誤 ({response.status_code}):\n{response.text}"
                                )
                            ...
                            if attempt < max_retries - 1:
                                ...
                                raise Exception("連線持續逾時 (Read timed out)。")
