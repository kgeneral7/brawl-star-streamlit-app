...
def get_initial_seeds(api_key, log_container):
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            log_message(f"❌ 嚴重錯誤 (403)：API 拒絕存取！請確認有將首頁掃描到的 IP 加進金鑰白名單中。", log_container)
...
def render_scraper():
    ...
    with st.sidebar:
        ...
        mode = st.radio("🎯 選擇收割模式", ["rooms", "solo"], index=0 if st.session_state.scraper_mode == "rooms" else 1, format_func=lambda x: "模式 A：神仙房陣容收割" if x == "rooms" else "模式 B：大神單人勝率")
    ...
    if not st.session_state.is_running:
        ...
        if st.session_state.data:
            ...
            st.download_button("⬇️ 點此下載 CSV 報表", data=csv_data.encode('utf-8-sig'), file_name=st.session_state.export_filename, mime="text/csv", use_container_width=True, type="primary")
