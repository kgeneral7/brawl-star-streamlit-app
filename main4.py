...
if not st.session_state.authenticated:
    ...
    # st.stop() 會強制停止這行以下的所有的程式碼，包含左側導覽列都不會顯示！
    st.stop()
...
if 'solo_data' not in st.session_state: st.session_state.solo_data = []   
...
def render_scraper():
    ...
    with st.sidebar:
        ...
        st.divider()
        st.header("⚡ 效能壓榨引擎")
        w_count = st.slider("🚀 併發核心數 (每個模式的分配量)", min_value=1, max_value=8, value=st.session_state.worker_count, step=1)
