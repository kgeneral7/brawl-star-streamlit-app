...
if 'worker_count' not in st.session_state: st.session_state.worker_count = 6 # 預設 6 核心
...
def harvest_rooms(headers, duration, worker_count, ctx):
    ...
    def worker(worker_id):
        ...
        while datetime.now() < end_time and st.session_state.is_running:
            try: current_tag = q.get(timeout=2)
            ...
            try:
                ...
                if res.status_code == 200:
                    ...
                    if prof_res.status_code == 200 and prof_res.json().get('3vs3Victories', 0) >= 5000:
                        elite_found += 1
                        if elite_found % 3 == 0:
                            log_message(f"{mode_tag} 🏆 核心 {worker_id} 捕獲神仙 {raw_tag}！")
    ...
    for i in range(worker_count):
        with GLOBAL_LOCK: st.session_state.active_tasks += 1
    ...
    def worker(worker_id):
        ...
        while datetime.now() < end_time and st.session_state.is_running:
            ...
            if worker_id == 0 and iteration % 5 == 0:
                elapsed = (datetime.now() - st.session_state.start_time).seconds / 60
                with data_lock:
                    total_samples = sum(s['played'] for md in stats.values() for mp in md.values() for s in mp.values())
            ...
            try:
                ...
                if res.status_code == 200:
                    ...
                    if prof_res.status_code == 200 and prof_res.json().get('3vs3Victories', 0) >= 5000:
                        elite_found += 1
                        if elite_found % 3 == 0:
                            log_message(f"{mode_tag} 🏆 核心 {worker_id} 捕獲大神 {raw_tag}！")
...
def render_home():
    ...
    with st.spinner("正在對雲端伺服器進行深度 IP 掃描 (約需 3 秒)..."):
        try:
            ...
            st.warning("⚠️ 【破解 403 必做】：請前往 Supercell Developer 後台，**將上面掃描出的「所有」IP 加進白名單（一行一個）**，然後把新產生的 API Key 貼到下方的輸入框中！")
        except Exception as e:
    ...
    with col1:
        ...
        if bs_input != st.session_state.bs_api_key:
            st.session_state.bs_api_key = bs_input.replace('"', '').replace("'", "").strip()
            st.rerun()
...
def render_scraper():
    ...
    with st.sidebar:
        ...
        st.divider()
        st.header("⚡ 效能壓榨引擎")
        w_count = st.slider("🚀 併發核心數 (每個模式的分配量)", min_value=1, max_value=6, value=st.session_state.worker_count, step=1)
        st.session_state.worker_count = w_count
        st.caption("⚠️ 核心數開越高抓越快，但過高容易觸發官方 `429` 封鎖，建議維持在 4~6 之間。")

    col1, col2, col3 = st.columns(3)
    with col1:
        if st.button("▶️ 啟動多核心收割陣列", disabled=st.session_state.is_running, use_container_width=True, type="primary"):
    ...
    if st.session_state.is_running:
        st.info(f"🔄 **多核心收割陣列全速運作中...** (目前有 {st.session_state.active_tasks} 個核心正在背景狂飆！)")
...
def render_bp():
    ...
    with st.sidebar:
        ...
        if st.session_state.all_brawlers:
            selected_brawlers = st.multiselect("選擇你擁有的英雄", options=sorted(list(st.session_state.all_brawlers)), default=list(st.session_state.owned_brawlers))
    ...
    with col_m1: selected_mode = st.selectbox("遊戲模式", sorted(list(st.session_state.mode_map_dict.keys())))
    with col_m2: selected_map = st.selectbox("地圖名稱", sorted(list(st.session_state.mode_map_dict.get(selected_mode, []))))
    with col_m3: first_pick = st.radio("開局首搶", ["我方先選 (藍方 First Pick)", "敵方先選 (紅方 First Pick)"])
        ...
        if not st.session_state.gemini_api_key:
            ...
            if selected_mode in st.session_state.condensed_data and selected_map in st.session_state.condensed_data[selected_mode]:
                for b, s in st.session_state.condensed_data[selected_mode][selected_map].items():
                    ...
                    if s["plays"] >= 3: stats_list.append({"name": b, "plays": s["plays"], "win_rate": s["win_rate"]})
            ...
            prompt_data = f"【情報】\n- 模式：{selected_mode}\n- 地圖：{selected_map}\n- 先手：{first_pick}\n- 敵選：{str_enemies}\n- 我選：{str_allies}\n- 禁用：{str_bans}\n\n【我方可用高勝率 Top 15】\n"
            for i, s in enumerate(top_winrate, 1): prompt_data += f"{s['name']} (勝率 {s['win_rate']}%, 出場 {s['plays']})\n"

            if not enemies and not allies and len(bans) < 6:
                task_instruction = "進入「禁用 (Ban) 階段」。根據資料庫找出3隻最該Ban的毒瘤，並考量先後手給出建議。"
            else:
                task_instruction = "進入「選角 (Pick) 階段」。挑出 1~3 隻能克制敵方且配合我方的可用英雄，精簡說明理由。"
            system_prompt = f"你是實戰教練，嚴格從可用清單挑選英雄。{task_instruction}"

            with st.status("🧠 呼叫 AI 中...", expanded=True) as status:
                try:
                    ...
                    if list_res.status_code != 200: raise Exception(f"API Key 錯誤 ({list_res.status_code})")
                    available_models = [m['name'].replace('models/', '') for m in list_res.json().get('models', []) if 'generateContent' in m.get('supportedGenerationMethods', []) and 'vision' not in m['name'].lower()]
                    ...
                    response = requests.post(url, headers={'Content-Type': 'application/json'}, json={"system_instruction": {"parts": [{"text": system_prompt}]}, "contents": [{"parts": [{"text": prompt_data}]}], "generationConfig": {"temperature": 0.5}}, timeout=45)
                    if response.status_code == 200:
                        st.success("🎯 戰術指示抵達！")
                        st.info(response.json()['candidates'][0]['content']['parts'][0]['text'])
                        status.update(label="✅ 解析完畢", state="complete", expanded=False)
...
st.sidebar.divider()
if page == "🏠 首頁大廳": render_home()
elif page == "🚀 排位數據收割機": render_scraper()
elif page == "🤖 BP 即時戰術指示器": render_bp()